...
В файле [lab2_ex.cpp](lab2_ex.cpp) приведен пример выполнения лабораторной работы для рассмотренного графа. Данный пример не проходит тесты, поскольку варианта с таким графом не существует. Скомпилировать и запустить данный пример можно с помощью последовательности команд
```
...
Группа потоков, выполняющихся без синхронизации или выполняющихся со строгим чередованием за счет использованиям семафоров, может включать в себя несколько интервалов, показанных на графе. Например:
```
