...
2. Выбрать граф запуска потоков в соответствии с вариантом задания. Вершины графа являются точками запуска/завершения потоков, дугами обозначены сами потоки, длину дуги следует интерпретировать как ориентировочное время выполнения потока. В процессе своей работы каждый поток должен в цикле выполнять два действия: выводить букву имени потока в консоль, после чего вызывать функцию `computation()` для выполнения сложных расчетов, требующих задействования ЦП на длительное время. Эта функция уже написана и подключается из заголовочного файла `lab2.h`, изменять ее не следует.
3. Самостоятельно выделить на графе две группы с выполняющимися параллельно потоками. Первая группа не синхронизирована, параллельное выполнение входящих в группу потоков происходит за счет планировщика задач (см. [примеры](examples/README.md) [1](examples/README.md#Организация-параллельного-выполнения-потоков-без-использования-средств-синхронизации) и [2](examples/README.md#Использование-мьютекса-для-работы-с-общим-ресурсом)). Вторая группа синхронизирована семафорами и потоки внутри группы выполняются в строго зафиксированном порядке: входящий в групу поток передает управление другому потоку после каждой итерации цикла (см. [пример 3](examples/README.md#Использование-семафоров-для-синхронизации-потоков)). Каждая из двух групп должна включать в себя не менее 1 интервала на графе, состоящего минимум из 3 потоков.
4. Реализовать программу для последовательно-параллельного выполнения потоков в ОС Linux или Mac OS X с использованием средств POSIX для запуска и синхронизации потоков. Запрещается использовать какие-либо библиотеки и модули, решающие задачу кроссплатформенной разработки многопоточных приложений (std::thread, Qt Thread, Boost Thread и т.п.). Для этого необходимо написать код в файле `lab2.cpp`:
    1. Функция `unsigned int lab2_thread_graph_id()` должна возвращать номер графа запуска потоков, полученный на шаге 2.
    2. Функция `const char* lab2_unsynchronized_threads()` должна возвращать разделенный запятой список интервалов, где потоки не синхронизированы (см. предыдущий пункт, а также примеры в файлах [lab2.cpp](lab2.cpp) и [lab2_ex.cpp](lab2_ex.cpp)).
    3. Функция `const char* lab2_sequential_threads()` должна возвращать разделенный запятой список интервалов, где потоки синхронизированы с помощью семафора (-ов) и выполняются в строгой последовательности (см. предыдущий пункт, а также примеры в файлах [lab2.cpp](lab2.cpp) и [lab2_ex.cpp](lab2_ex.cpp)).
...
5. Последовательное выполнение потоков может обеспечиваться как за счет использования семафоров, так и с помощью функции `pthread_join()`. Запускать потоки можно как все сразу в функции `lab2_init()`, так и по одному (или группами) из других потоков.
...
В файле [lab2_ex.cpp] приведен пример выполнения лабораторной работы для рассмотренного графа. Данный пример не проходит тесты, поскольку варианта с таким графом не существует. Скомпилировать и запустить данный пример можно с помощью последовательности команд
```
...
## Примеры
Перед выполнением лабораторной работы рекомендуется ознакомиться с [примерами](examples/README.md) синхронизации потоков. Также возможный подход к выполнению лабораторной работы показан в файле [lab2_ex.cpp](lab2_ex.cpp). В данном примере потоки b, c и d не синхронизированы, а потоки d и e синхронизированы парой мьютексов и выполняются в строгой последовательности (чередуются) по аналогии с задачей производителя-потребителя.

Группа потоков, выполняющихся без синхронизации или выполняющихся со строгим чередованием за счет использованиям мьютексов, может включать в себя несколько интервалов, показанных на графе. Например:
```
...
```
В данном случае предполагается, что потоки a, b и c синхронизированы друг с другом и выполняются со строгим чередованием в одном интервале. Потоки c, d, e и f выполняются параллельно в другом интервале на графе и также синхронизированы семафорами для создания строгого чередования между ними. Наконец, параллельно выполняющиеся потоки на интервале hkm также синхронизированы семафорами и выполняются в строгой очередности. Вывод программы может иметь следующий вид: `abcabcabc defcdefcdefc dfggfdgdf kmhkmhkmh`
...
```
Здесь три раза выводится буква потока (_a_), что соответствует дуге графа длиной в один интервал. Команда `flush` используется для немедленного вывода буквы потока в консоль, чтобы избежать буферизации на уровне операционной системы. После вывода каждой буквы в консоль обязательно вызывается функция `computation()` для имитации проведения вычислений и выполнения ресурсо-затратных операций, в большинстве случаев приводящих к переключению контекста до того, как данным потоком в консоль будет выведена следующая буква.
