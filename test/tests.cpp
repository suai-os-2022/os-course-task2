...
TEST(lab2_tests, tasknumber) {
    EXPECT_GE(lab2_thread_graph_id(), 1)  << "Thread graph must be greater than or equal to 1";
    EXPECT_LE(lab2_thread_graph_id(), 20) << "Thread graph must be less than or equal to 20";
    std::cout << "TASKID is " << lab2_thread_graph_id() << std::endl;
}
...
TEST(lab2_tests, unsynchronizedthreads) {
    ...
    std::cout << "Unsynchronized threads are " << lab2_unsynchronized_threads() << std::endl;
}
...
TEST(lab2_tests, sequentialthreads) {
    ...
    std::cout << "Sequential threads are " << lab2_sequential_threads() << std::endl;
}


TEST(lab2_tests, threadsync) {
    std::vector< std::string > solution;
    int solution_len = 0;
...
//    std::cout << "Running tests for lab2 ..." << std::endl;
    ...
    // call student's lab2 code
    int n = lab2_init();
    std::cout.rdbuf(coutbuf); //reset to standard output again
    EXPECT_EQ(n, 0) << "If 'lab2_init()' succeeds, it should return 0.";
