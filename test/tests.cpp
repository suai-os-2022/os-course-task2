...
TEST(lab2_tests, tasknumber) {
    EXPECT_GE(lab2_task_number(), 1) << "Task number must be greater than or equal to 1";
    EXPECT_LE(lab2_task_number(), 20) << "Task number must be less than or equal to 20";
}
...
TEST(lab2_tests, threadsync) {
    ...
    filename << "task" << lab2_task_number() << ".txt";
...
//        std::cout << c_cur <<", " << c_prev << ", " << interval << ", " << solution.at(interval) << ", " << is_parallel << std::endl;
        if (solution.at(interval).find(c_cur) != std::string::npos) {
            if (c_cur != c_prev and iterval_stats[c_cur] > 0)
                is_parallel = true;
